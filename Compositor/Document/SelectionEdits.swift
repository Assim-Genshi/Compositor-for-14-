...
extension EditorSession {
    ...
    func fillSelection(with source: FillSource) async {
        ...
        await applyPixelEdit(to: layer, name: isMaskSelected ? String(localized: "Fill Mask") : String(localized: "Fill")) { try $0.fill(color) }
    }
    ...
    func invertPixels() async {
        ...
        do {
            ...
            beginEdit(mask ? String(localized: "Invert Mask") : String(localized: "Invert"))
            if mask {
    ...
    func finishPixelMove() async {
        ...
        if move.offset != .zero {
            ...
            do { try await commitRasterEdit(move.raster, name: move.duplicate ? String(localized: "Duplicate Pixels") : String(localized: "Move Pixels")) { self.document?.selection = moved } }
            catch { brushError = error.localizedDescription }
        }
        pixelMove = nil
        brushRevision += 1
