...
struct ContentView: View {
    ...
    /// Kept out of `body`, whose type-checking is already near its limit.
    ...
    private var statusHint: String {
        ...
        case .shape:
            String(localized: "Drag to draw a shape on a new layer · Shift \(session.shapeKind == .line ? "45°" : session.shapeKind == .rectangle ? "square" : "circle") · Option from center · Shift-U or Tab for the next shape · Escape cancel · Space to pan")
        case .gradient:
...
private extension ToolbarContent {
    /// Drops the glass background macOS 26 draws behind toolbar items; earlier toolbars draw none.
...
private extension View {
    @ViewBuilder
    func columnResizePointer() -> some View {
